...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    let xLength = Math.sqrt(x1*x1+y1*y1);
    //Длина второго вектора
   let yLength =  Math.sqrt(x2*x2+y2*y2);
   //  Скалярное произведение
  let multiply =x1*x2+y1*y2;
  //Косинус угла
  let angleCos= multiply/(xLength*yLength);
  console.log(angleCos);
  if(angleCos === 0) return Math.PI/2;
  if(angleCos === 1) return 0;
  if(angleCos === -1) return Math.PI;
let res ;
if( angleCos > 0)
   res = Math.atan(Math.sqrt(1-Math.sqrt(angleCos))/angleCos);
  else {
    res = Math.PI + Math.atan(Math.sqrt(1-Math.sqrt(angleCos))/angleCos);
  }
  return res;
}
...
function toNumber(value, def) {
    ...
    return isNumeric(value)? +value : def;
}
